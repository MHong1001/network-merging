...
def multi_pass_aug_mean(args, model1, model2, device, test_loader):
    result = []
    for i in range(len(AUGS)):
        a = AUGS[i]
        print(f"Applied methods: {AUGS_DESCP[i]}, Agg Method: mean")
        test_loss, acc = mean_agg_test(args, model1, model2, a, device, test_loader)
        result.append({"methods": AUGS_DESCP[i], "test_loss": test_loss, "acc": acc})

    return result


def multi_pass_aug_voting(args, model1, model2, device, test_loader):
    result = []
    for i in range(len(AUGS)):
        a = AUGS[i]
        print(f"Applied methods: {AUGS_DESCP[i]}, Agg Method: voting")
        test_loss, acc = voting_agg_test(args, model1, model2, a, device, test_loader)
        result.append({"methods": AUGS_DESCP[i], "test_loss": test_loss, "acc": acc})
