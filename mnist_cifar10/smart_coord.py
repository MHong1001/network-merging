...
def predict_with_logits_pan(args, model1, model2, upan, m1_data, m2_data):
    """
...
def predict_with_feature_pan(args, model1, model2, pan1, pan2, m1_data, m2_data):
    """
    Make a prediction with PAN using features of the models.
    Here we take a winner takes all approach, as we have 2 classifier classifying 1 input with
    1 intended label(output). However, theoredically we can also go for a multi-label(multi-output)
...
def predict_with_agnostic_pan(args, model1, model2, upan, m1_data, m2_data):
    """
...
def smart_coord_test(args, model1, model2, upan, device, test_loaders):
    model1.eval()
    model2.eval()
    upan.eval()
    test_loss = 0
    ...
    for test_loader in test_loaders:
        for m1_data, m2_data, target in test_loader:
            ...
            if args.upan_type == "feature":
                output = predict_with_feature_pan(
                    args, model1, model2, upan, upan, m1_data, m2_data
                )
            elif args.upan_type == "logits":
                output = predict_with_logits_pan(
                    args, model1, model2, upan, m1_data, m2_data
                )
            elif (
                args.upan_type == "agnostic_feature"
                or args.upan_type == "agnostic_logits"
            ):
                output = predict_with_agnostic_pan(
                    args, model1, model2, upan, m1_data, m2_data
                )
            else:
